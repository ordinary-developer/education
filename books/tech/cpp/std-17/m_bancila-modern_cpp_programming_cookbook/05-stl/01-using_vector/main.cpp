...
#include <iostream>
#include <cassert>
namespace example_01 { // -> initialization of std::vector
...
#include <iostream>
#include <cassert>
namespace example_02 { // -> modification of std::vector
...
void run() {
    ...
    assert((not v3.empty() and 1 == v3.size() and 1 == v3.begin()->a and "one"s == v3.begin()->c));
    
    v3.emplace(v3.begin(), 2, 2.0, "two"s);
    assert((
        not v3.empty() and 2 == v3.size()
        and 2 == v3[0].a and "two"s == v3[0].c
        and 1 == v3[1].a and "one"s == v3[1].c)); 
...
#include <iostream>
#include <cassert>
namespace example_03 { // modifying the whole std::vector
void run() {
    std::vector<int> const v1{ 1, 2, 3, 4, 5 };
...
#include <iostream>
#include <cassert>
namespace example_05 { // reserving space for std::vector
void run() {
    std::vector<int> v1{ 1, 2, 3, 4, 5 };
    v1.reserve(10);
