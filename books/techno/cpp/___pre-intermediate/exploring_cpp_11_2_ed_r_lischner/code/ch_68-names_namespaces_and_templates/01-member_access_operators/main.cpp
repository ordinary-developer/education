...
class point3d : public point2d<T> {
    public:
        ...
        virtual ~point3d() final override = default;
        ...
        virtual void print(std::ostream& stream) const final override {
            stream << '(' 
                   << this->x() << ", " 
                   << this->y() << ", " 
                   << z()       << ')';
...
template <class T>
std::ostream& operator<<(std::ostream& stream, point2d<T> const& point) {
    point.print(stream);
    return stream;
}
