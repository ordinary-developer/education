...
void SyntaxHighlighter::highlightBlock(const QString& str) {
    ...
    for (int i{0}; i < str.length(); ++i) {
        if (InsideCStyleComment == nState) {
        ...
        else if (InsideCString == nState) {
            if ("\"" == str.mid(i, 1) or "\'" == str.mid(i, 1)) {
                if ("\\\"" != str.mid(i - 1, 2) and 
                    "\\\'" != str.mid(i - 1, 2))
                {
                    nState = NormalState;
            ...
            else if ("\"" == str.mid(i, 1) or "\'" == str.mid(i, 1)) {
                nStart = i;
