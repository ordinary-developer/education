...
int CMainWindow::OnCreate(LPCREATESTRUCT lpcs) {
	...
	m_wndLBTitle.Create(_T("Typefaces"), WS_CHILD | WS_VISIBLE | SS_LEFT, rect, this);

	rect.SetRect(m_cxChar * 2, m_cyChar * 2, m_cxChar * 48, m_cyChar * 18);
	m_wndListBox.CreateEx(
		WS_EX_CLIENTEDGE, 
		_T("listbox"),
		NULL,
...
void CMainWindow::OnPushButtonClicked() {
	MessageBox(
		_T("This feature is currently unimplemented. Sorry!"),
		_T("Error"),
...
void CMainWindow::OnSelChange() {
	...
	if (LB_ERR == nIndex) {
		...
		m_wndSampleText.SetWindowTextW(_T(""));
	}
	else {
		m_wndPushButton.EnableWindow(TRUE);
		if (NULL != (HFONT)m_fontSample)
		...
		m_wndSampleText.SetWindowTextW(_T("AaBbCcDdEeFfGg"));
	}
}
...
void CMainWindow::FillListBox()
{
	...
	CClientDC dc(this);
	::EnumFontFamilies(
		(HDC)dc, NULL, (FONTENUMPROC)EnumFontFamProc, (LPARAM)this);
}

int CALLBACK CMainWindow::EnumFontFamProc(
	ENUMLOGFONT* lpelf, NEWTEXTMETRIC* lpntm, int nFontType, LPARAM lParam)
{
	CMainWindow* pWnd = (CMainWindow*)lParam;

	if ((BST_UNCHECKED == pWnd->m_wndCheckBox.GetCheck())
