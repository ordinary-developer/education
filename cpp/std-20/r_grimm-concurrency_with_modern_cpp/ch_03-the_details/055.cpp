...
constexpr long long size = 25'000'000;
...
int main() {
    ...
    getExecutionTime("std::execution::seq", [workVec]() mutable {
        std::transform(std::execution::seq, std::begin(workVec), std::end(workVec), std::begin(workVec),
            [](double arg) { return std::tan(arg); });
    });
    getExecutionTime("std::execution::par", [workVec]() mutable {
        std::transform(std::execution::par, std::begin(workVec), std::end(workVec), std::begin(workVec),
            [](double arg) { return std::tan(arg); });
    });
    getExecutionTime("std::execution::par_unseq", [workVec]() mutable {
        std::transform(std::execution::par_unseq, std::begin(workVec), std::end(workVec), std::begin(workVec),
            [](double arg) { return std::tan(arg); });
    });
}
